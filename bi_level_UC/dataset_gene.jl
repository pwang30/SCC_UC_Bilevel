...
# include("admittance_matrix_calculation.jl")
    ...
    for k in 1:size(SGpara,1)                      # calculate the ADMITTANCE MATRIX of the SGs
        for j in 2:size(SGpara,2)
            Y_SGs[k, j-1] = 1/SGpara[k, j]/2         
        end           
    end 
    
    E_sg=1                   # voltage of the SGs in p.u., set as 1 p.u.
    E_sg=β*E_sg
    ...
    for j in 1:numnodes
        for k in 1:Ω
            ...
            for i in 1:length(SGpara[:,2:end])
                Y_SGs_with_status[Int(SGpara[Int(ceil(i/2)), 1]), Int(SGpara[Int(ceil(i/2)), 1])] = Y_SGs[Int(ceil(i/2)),index] * status_SGs[i]  # status of SGs
                    index = index+1
   ...
   # writedlm("/Users/kl/Desktop/single_level_UC_SCL/I_SCC_all_buses_scenarios.csv", I_SCC_all_buses_scenarios, ',')
   # writedlm("/Users/kl/Desktop/single_level_UC_SCL/matrix_ω.csv", matrix_ω, ',')
