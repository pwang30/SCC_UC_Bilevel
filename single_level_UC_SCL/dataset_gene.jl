...
Y_sg_1 = zeros(numnodes, numnodes)      # define admittance matrix of the SGs
Y_sg_2 = zeros(numnodes, numnodes)
Y_sg_3 = zeros(numnodes, numnodes)
I_sg_1 =zeros(1,size(SGpara, 1))
I_sg_2 =zeros(1,size(SGpara, 1))
I_sg_3 =zeros(1,size(SGpara, 1))
...
for k in 1:size(SGpara, 1)                  # calculate the ADMITTANCE MATRIX of the SGs
    ...
    Y_sg_1[i, i] = 1/SGpara[k, 2]/2               # diagonal elements 
    Y_sg_2[i, i] = 1/SGpara[k, 3]/2              
    Y_sg_3[i, i] = 1/SGpara[k, 4]/2               
end 
...
for k in 1:size(SGpara, 1)
    I_sg_1[k]=E_sg/SGpara[k, 2]/2
    I_sg_2[k]=E_sg/SGpara[k, 3]/2
    I_sg_3[k]=E_sg/SGpara[k, 4]/2
end


#------------Evaluate the SCC at fault bus F, let's say F=4, for the generator in bus 4
Ω=2^(size(SGpara, 1)*3+num_IBG)    # dimension of the data set, which is generated by evaluating SCC
I_scc_all=zeros(Ω,numnodes)            # SCC at fault bus F=4
ηm=zeros(Ω,15)                   # product of each pair of SGs

combinations = collect(product(0:1, repeat([0:1], 20)...))   # generate all combinations of 9 binary variables
...
for j in 1:numnodes
    for k in 1:Ω
        ...
        for i in 1:size(SGpara, 1)
            Yₛ₉_status[SGpara[i, 1], SGpara[i, 1]] = 1/SGpara[i, 2]/2 * status_SGs[i]  # status of SGs
        end

        Y_total .= Yₗᵢₙₑ + Yₛ₉_status  # calculate the total admittance matrix
        Z .= inv(Y_total)  # calculate the impedance matrix

        I_scc_all[k,j] = (Z[j,2]*Iₛ₉[1]*status_SGs[1]+ Z[j,3]*Iₛ₉[2]*status_SGs[2]+ Z[j,4]*Iₛ₉[3]*status_SGs[3]+
        Z[j,5]*Iₛ₉[4]*status_SGs[4]+ Z[j,27]*Iₛ₉[5]*status_SGs[5]+ Z[j,30]*Iₛ₉[6]*status_SGs[6]+
        Z[j,1]*I_IBG*status_IBG[1]+ Z[j,23]*I_IBG*status_IBG[2]+ Z[j,26]*I_IBG*status_IBG[3])/Z[j,j]   # calculate the SCC at fault bus F=4
    end
end


return I_scc_all, matrix_ω
