...
@constraint(model, Oᵐ₁[2]*kᵐ[T] -λᴱ[T] +μᵐᵃˣˢᴳ³_1[T] -μᵐⁱⁿˢᴳ³_1[T] +πʳᵘˢᴳ³_1[T] -πʳᵈˢᴳ³_1[T]>=0)
@constraint(model, Oᵐ₂[2]*kᵐ[T] -λᴱ[T] +μᵐᵃˣˢᴳ³_2[T] -μᵐⁱⁿˢᴳ³_2[T] +πʳᵘˢᴳ³_2[T] -πʳᵈˢᴳ³_2[T]>=0)
...
for t in 1:T-1                                                                                                  # dual constraints for generation, when t<=T-1, assume SGs in bus 2 are strategic
    ...
    @constraint(model, Oᵐ₁[2]*kᵐ[t] -λᴱ[t] +μᵐᵃˣˢᴳ³_1[t] -μᵐⁱⁿˢᴳ³_1[t] +πʳᵘˢᴳ³_1[t] -πʳᵘˢᴳ³_1[t+1] -πʳᵈˢᴳ³_1[t] +πʳᵈˢᴳ³_1[t+1]>=0)                
    @constraint(model, Oᵐ₂[2]*kᵐ[t] -λᴱ[t] +μᵐᵃˣˢᴳ³_2[t] -μᵐⁱⁿˢᴳ³_2[t] +πʳᵘˢᴳ³_2[t] -πʳᵘˢᴳ³_2[t+1] -πʳᵈˢᴳ³_2[t] +πʳᵈˢᴳ³_2[t+1]>=0)
...
for t in 1:T
    ...
    @constraint(model, z₁[t]>=λᴱ[t]*Pˢᴳₘᵢₙ[2])
    @constraint(model, z₁[t]>=M_λ*Pˢᴳ³_1[t]   +Pˢᴳₘₐₓ[2]*(λᴱ[t]-M_λ) )
    @constraint(model, z₁[t]<=λᴱ[t]*Pˢᴳₘₐₓ[2])
    @constraint(model, z₁[t]<=M_λ*Pˢᴳ³_1[t]   +Pˢᴳₘᵢₙ[2]*(λᴱ[t]-M_λ) )
    
    @constraint(model, z₂[t]>=λᴱ[t]*Pˢᴳₘᵢₙ[2])
    @constraint(model, z₂[t]>=M_λ*Pˢᴳ³_2[t]   +Pˢᴳₘₐₓ[2]*(λᴱ[t]-M_λ) )
    @constraint(model, z₂[t]<=λᴱ[t]*Pˢᴳₘₐₓ[2])
    @constraint(model, z₂[t]<=M_λ*Pˢᴳ³_2[t]   +Pˢᴳₘᵢₙ[2]*(λᴱ[t]-M_λ)  )

    @constraint(model, z₃[t]>=Pˢᴳ³_1[t]+kᵐ[t]*Pˢᴳₘᵢₙ[2]-Pˢᴳₘᵢₙ[2])
    @constraint(model, z₃[t]>=kᵐᵐᵃˣ*Pˢᴳ³_1[t]+kᵐ[t]*Pˢᴳₘₐₓ[2]-kᵐᵐᵃˣ*Pˢᴳₘₐₓ[2])
    @constraint(model, z₃[t]<=kᵐᵐᵃˣ*Pˢᴳ³_1[t]+kᵐ[t]*Pˢᴳₘᵢₙ[2]-kᵐᵐᵃˣ*Pˢᴳₘᵢₙ[2])
    @constraint(model, z₃[t]<=Pˢᴳ³_1[t]+kᵐ[t]*Pˢᴳₘₐₓ[2]-Pˢᴳₘₐₓ[2])

    @constraint(model, z₄[t]>=Pˢᴳ³_2[t]+kᵐ[t]*Pˢᴳₘᵢₙ[2]-Pˢᴳₘᵢₙ[2])
    @constraint(model, z₄[t]>=kᵐᵐᵃˣ*Pˢᴳ³_2[t]+kᵐ[t]*Pˢᴳₘₐₓ[2]-kᵐᵐᵃˣ*Pˢᴳₘₐₓ[2])
    @constraint(model, z₄[t]<=kᵐᵐᵃˣ*Pˢᴳ³_2[t]+kᵐ[t]*Pˢᴳₘᵢₙ[2]-kᵐᵐᵃˣ*Pˢᴳₘᵢₙ[2])
    @constraint(model, z₄[t]<=Pˢᴳ³_2[t]+kᵐ[t]*Pˢᴳₘₐₓ[2]-Pˢᴳₘₐₓ[2])
end
...
cost_nl_UL=sum(Oⁿˡ[2].*(yˢᴳ³_1+yˢᴳ³_2))                       # no-load cost of strategic SGs
cost_gene_UL=sum(Oᵐ₁[2].*Pˢᴳ³_1+Oᵐ₂[2].*Pˢᴳ³_2)               # generation cost of strategic SGs 
...
cost_gene_LL=Oᵐ₁[2]*sum( z₃ )+Oᵐ₂[2]*sum( z₄ )+sum(Oᵐ₁[5].*Pˢᴳ²⁷_1)+ sum(Oᵐ₂[5].*Pˢᴳ²⁷_2)+sum(Oᵐ₁[3].*Pˢᴳ⁴_1)+ sum(Oᵐ₂[3].*Pˢᴳ⁴_2)+sum(Oᵐ₁[4].*Pˢᴳ⁵_1)+ sum(Oᵐ₂[4].*Pˢᴳ⁵_2)+sum(Oᵐ₁[1].*Pˢᴳ²_1)+ sum(Oᵐ₂[1].*Pˢᴳ²_2)+sum(Oᵐ₁[6].*Pˢᴳ³⁰_1)+sum(Oᵐ₂[6].*Pˢᴳ³⁰_2)   
...
matwrite("revenue_energy_UL_bus3.mat", Dict("revenue_energy_UL_bus3" => revenue_energy_UL))
matwrite("revenue_SCL_UL_bus3.mat", Dict("revenue_SCL_UL_bus3" => revenue_SCL_UL))
