...
@constraint(model, Oᵐ₁[3]*kᵐ[T] -λᴱ[T] +μᵐᵃˣˢᴳ⁴_1[T] -μᵐⁱⁿˢᴳ⁴_1[T] +πʳᵘˢᴳ⁴_1[T] -πʳᵈˢᴳ⁴_1[T]>=0)
@constraint(model, Oᵐ₂[3]*kᵐ[T] -λᴱ[T] +μᵐᵃˣˢᴳ⁴_2[T] -μᵐⁱⁿˢᴳ⁴_2[T] +πʳᵘˢᴳ⁴_2[T] -πʳᵈˢᴳ⁴_2[T]>=0)
...
for t in 1:T-1                                                                                                  # dual constraints for generation, when t<=T-1, assume SGs in bus 2 are strategic
    ...
    @constraint(model, Oᵐ₁[3]*kᵐ[t] -λᴱ[t] +μᵐᵃˣˢᴳ⁴_1[t] -μᵐⁱⁿˢᴳ⁴_1[t] +πʳᵘˢᴳ⁴_1[t] -πʳᵘˢᴳ⁴_1[t+1] -πʳᵈˢᴳ⁴_1[t] +πʳᵈˢᴳ⁴_1[t+1]>=0)
    @constraint(model, Oᵐ₂[3]*kᵐ[t] -λᴱ[t] +μᵐᵃˣˢᴳ⁴_2[t] -μᵐⁱⁿˢᴳ⁴_2[t] +πʳᵘˢᴳ⁴_2[t] -πʳᵘˢᴳ⁴_2[t+1] -πʳᵈˢᴳ⁴_2[t] +πʳᵈˢᴳ⁴_2[t+1]>=0)
...
for t in 1:T
    ...
    @constraint(model, z₁[t]>=λᴱ[t]*Pˢᴳₘᵢₙ[3])
    @constraint(model, z₁[t]>=M_λ*Pˢᴳ⁴_1[t]   +Pˢᴳₘₐₓ[3]*(λᴱ[t]-M_λ) )
    @constraint(model, z₁[t]<=λᴱ[t]*Pˢᴳₘₐₓ[3])
    @constraint(model, z₁[t]<=M_λ*Pˢᴳ⁴_1[t]   +Pˢᴳₘᵢₙ[3]*(λᴱ[t]-M_λ) )
    
    @constraint(model, z₂[t]>=λᴱ[t]*Pˢᴳₘᵢₙ[3])
    @constraint(model, z₂[t]>=M_λ*Pˢᴳ⁴_2[t]   +Pˢᴳₘₐₓ[3]*(λᴱ[t]-M_λ) )
    @constraint(model, z₂[t]<=λᴱ[t]*Pˢᴳₘₐₓ[3])
    @constraint(model, z₂[t]<=M_λ*Pˢᴳ⁴_2[t]   +Pˢᴳₘᵢₙ[3]*(λᴱ[t]-M_λ)  )

    @constraint(model, z₃[t]>=Pˢᴳ⁴_1[t]+kᵐ[t]*Pˢᴳₘᵢₙ[3]-Pˢᴳₘᵢₙ[3])
    @constraint(model, z₃[t]>=kᵐᵐᵃˣ*Pˢᴳ⁴_1[t]+kᵐ[t]*Pˢᴳₘₐₓ[3]-kᵐᵐᵃˣ*Pˢᴳₘₐₓ[3])
    @constraint(model, z₃[t]<=kᵐᵐᵃˣ*Pˢᴳ⁴_1[t]+kᵐ[t]*Pˢᴳₘᵢₙ[3]-kᵐᵐᵃˣ*Pˢᴳₘᵢₙ[3])
    @constraint(model, z₃[t]<=Pˢᴳ⁴_1[t]+kᵐ[t]*Pˢᴳₘₐₓ[3]-Pˢᴳₘₐₓ[3])

    @constraint(model, z₄[t]>=Pˢᴳ⁴_2[t]+kᵐ[t]*Pˢᴳₘᵢₙ[3]-Pˢᴳₘᵢₙ[3])
    @constraint(model, z₄[t]>=kᵐᵐᵃˣ*Pˢᴳ⁴_2[t]+kᵐ[t]*Pˢᴳₘₐₓ[3]-kᵐᵐᵃˣ*Pˢᴳₘₐₓ[3])
    @constraint(model, z₄[t]<=kᵐᵐᵃˣ*Pˢᴳ⁴_2[t]+kᵐ[t]*Pˢᴳₘᵢₙ[3]-kᵐᵐᵃˣ*Pˢᴳₘᵢₙ[3])
    @constraint(model, z₄[t]<=Pˢᴳ⁴_2[t]+kᵐ[t]*Pˢᴳₘₐₓ[3]-Pˢᴳₘₐₓ[3])
end
...
cost_nl_UL=sum(Oⁿˡ[3].*(yˢᴳ⁴_1+yˢᴳ⁴_2))                       # no-load cost of strategic SGs
cost_gene_UL=sum(Oᵐ₁[3].*Pˢᴳ⁴_1+Oᵐ₂[3].*Pˢᴳ⁴_2)               # generation cost of strategic SGs 
...
cost_gene_LL=Oᵐ₁[3]*sum( z₃ )+Oᵐ₂[3]*sum( z₄ )+sum(Oᵐ₁[5].*Pˢᴳ²⁷_1)+ sum(Oᵐ₂[5].*Pˢᴳ²⁷_2)+sum(Oᵐ₁[2].*Pˢᴳ³_1)+ sum(Oᵐ₂[2].*Pˢᴳ³_2)+sum(Oᵐ₁[4].*Pˢᴳ⁵_1)+ sum(Oᵐ₂[4].*Pˢᴳ⁵_2)+sum(Oᵐ₁[1].*Pˢᴳ²_1)+ sum(Oᵐ₂[1].*Pˢᴳ²_2)+sum(Oᵐ₁[6].*Pˢᴳ³⁰_1)+sum(Oᵐ₂[6].*Pˢᴳ³⁰_2)   
...
matwrite("revenue_energy_UL_bus4.mat", Dict("revenue_energy_UL_bus4" => revenue_energy_UL))
matwrite("revenue_SCL_UL_bus4.mat", Dict("revenue_SCL_UL_bus4" => revenue_SCL_UL))
