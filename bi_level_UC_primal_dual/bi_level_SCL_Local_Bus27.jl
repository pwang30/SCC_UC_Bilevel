...
@constraint(model, Oᵐ₁[5]*kᵐ[T] -λᴱ[T] +μᵐᵃˣˢᴳ²⁷_1[T] -μᵐⁱⁿˢᴳ²⁷_1[T] +πʳᵘˢᴳ²⁷_1[T] -πʳᵈˢᴳ²⁷_1[T]>=0)
@constraint(model, Oᵐ₂[5]*kᵐ[T] -λᴱ[T] +μᵐᵃˣˢᴳ²⁷_2[T] -μᵐⁱⁿˢᴳ²⁷_2[T] +πʳᵘˢᴳ²⁷_2[T] -πʳᵈˢᴳ²⁷_2[T]>=0)
...
for t in 1:T-1                                                                                                  # dual constraints for generation, when t<=T-1, assume SGs in bus 2 are strategic
    ...
    @constraint(model, Oᵐ₁[5]*kᵐ[t] -λᴱ[t] +μᵐᵃˣˢᴳ²⁷_1[t] -μᵐⁱⁿˢᴳ²⁷_1[t] +πʳᵘˢᴳ²⁷_1[t] -πʳᵘˢᴳ²⁷_1[t+1] -πʳᵈˢᴳ²⁷_1[t] +πʳᵈˢᴳ²⁷_1[t+1]>=0)
    @constraint(model, Oᵐ₂[5]*kᵐ[t] -λᴱ[t] +μᵐᵃˣˢᴳ²⁷_2[t] -μᵐⁱⁿˢᴳ²⁷_2[t] +πʳᵘˢᴳ²⁷_2[t] -πʳᵘˢᴳ²⁷_2[t+1] -πʳᵈˢᴳ²⁷_2[t] +πʳᵈˢᴳ²⁷_2[t+1]>=0)
...
for t in 1:T
    ...
    @constraint(model, z₁[t]>=λᴱ[t]*Pˢᴳₘᵢₙ[5])
    @constraint(model, z₁[t]>=M_λ*Pˢᴳ²⁷_1[t]   +Pˢᴳₘₐₓ[5]*(λᴱ[t]-M_λ) )
    @constraint(model, z₁[t]<=λᴱ[t]*Pˢᴳₘₐₓ[5])
    @constraint(model, z₁[t]<=M_λ*Pˢᴳ²⁷_1[t]   +Pˢᴳₘᵢₙ[5]*(λᴱ[t]-M_λ) )
    
    @constraint(model, z₂[t]>=λᴱ[t]*Pˢᴳₘᵢₙ[5])
    @constraint(model, z₂[t]>=M_λ*Pˢᴳ²⁷_2[t]   +Pˢᴳₘₐₓ[5]*(λᴱ[t]-M_λ) )
    @constraint(model, z₂[t]<=λᴱ[t]*Pˢᴳₘₐₓ[5])
    @constraint(model, z₂[t]<=M_λ*Pˢᴳ²⁷_2[t]   +Pˢᴳₘᵢₙ[5]*(λᴱ[t]-M_λ)  )

    @constraint(model, z₃[t]>=Pˢᴳ²⁷_1[t]+kᵐ[t]*Pˢᴳₘᵢₙ[5]-Pˢᴳₘᵢₙ[5])
    @constraint(model, z₃[t]>=kᵐᵐᵃˣ*Pˢᴳ²⁷_1[t]+kᵐ[t]*Pˢᴳₘₐₓ[5]-kᵐᵐᵃˣ*Pˢᴳₘₐₓ[5])
    @constraint(model, z₃[t]<=kᵐᵐᵃˣ*Pˢᴳ²⁷_1[t]+kᵐ[t]*Pˢᴳₘᵢₙ[5]-kᵐᵐᵃˣ*Pˢᴳₘᵢₙ[5])
    @constraint(model, z₃[t]<=Pˢᴳ²⁷_1[t]+kᵐ[t]*Pˢᴳₘₐₓ[5]-Pˢᴳₘₐₓ[5])

    @constraint(model, z₄[t]>=Pˢᴳ²⁷_2[t]+kᵐ[t]*Pˢᴳₘᵢₙ[5]-Pˢᴳₘᵢₙ[5])
    @constraint(model, z₄[t]>=kᵐᵐᵃˣ*Pˢᴳ²⁷_2[t]+kᵐ[t]*Pˢᴳₘₐₓ[5]-kᵐᵐᵃˣ*Pˢᴳₘₐₓ[5])
    @constraint(model, z₄[t]<=kᵐᵐᵃˣ*Pˢᴳ²⁷_2[t]+kᵐ[t]*Pˢᴳₘᵢₙ[5]-kᵐᵐᵃˣ*Pˢᴳₘᵢₙ[5])
    @constraint(model, z₄[t]<=Pˢᴳ²⁷_2[t]+kᵐ[t]*Pˢᴳₘₐₓ[5]-Pˢᴳₘₐₓ[5])
end
...
cost_nl_UL=sum(Oⁿˡ[5].*(yˢᴳ²⁷_1+yˢᴳ²⁷_2))                       # no-load cost of strategic SGs
cost_gene_UL=sum(Oᵐ₁[5].*Pˢᴳ²⁷_1+Oᵐ₂[5].*Pˢᴳ²⁷_2)               # generation cost of strategic SGs 
...
cost_gene_LL=Oᵐ₁[5]*sum( z₃ )+Oᵐ₂[5]*sum( z₄ )+sum(Oᵐ₁[2].*Pˢᴳ³_1+Oᵐ₂[2].*Pˢᴳ³_2)+sum(Oᵐ₁[3].*Pˢᴳ⁴_1+Oᵐ₂[3].*Pˢᴳ⁴_2)+sum(Oᵐ₁[4].*Pˢᴳ⁵_1+Oᵐ₂[4].*Pˢᴳ⁵_2)+sum(Oᵐ₁[1].*Pˢᴳ²_1+Oᵐ₂[1].*Pˢᴳ²_2)+sum(Oᵐ₁[6].*Pˢᴳ³⁰_1+Oᵐ₂[6].*Pˢᴳ³⁰_2)   
...
matwrite("revenue_energy_UL_bus27.mat", Dict("revenue_energy_UL_bus27" => revenue_energy_UL))
matwrite("revenue_SCL_UL_bus27.mat", Dict("revenue_SCL_UL_bus27" => revenue_SCL_UL))
